...
<p align="center">
  ...
  Ця автоматизація створена щоб полегшити життя ІТ-спеціалістам які вручну створюють облікові записи для користувачів в Active Directory. Скрипт бере данні які ввів HR в Microsoft Forms обробляє їх та створює обліковий запис в AD, надсилає лист ІТ-wellcome керівнику, та короткий звіт в приватний канал Microsoft Teams.
</p>
...
3. [Налаштування додатку в Entra](#налаштування-додатку-в-entra)
4. [Налаштування планувальника задач (автоматичний запуск скрипта)](#налаштування-планувальника-задач-автоматичний-запуск-скрипта)
5. [Налаштування Cloudflare Tunnel](#налаштування-cloudflare-tunnel)
6. [Налаштування Microsoft Forms](#налаштування-microsoft-forms)
7. [Налаштування Power Automate](#налаштування-power-automate)



## Підготовка віртуальної машини для запуску проекту
Потрібно розгорнути та базово налаштувти віртуальну машину на сервері, підійде ОС Windows 10/11 x64.
На розгорнутій ОС потрібно встановити Python, RSAT та LibreOffice (для редагування та конвертування файлу ІТ велком). Відкрийте PowerShell з правами адміністратора та запустіть наступні команди:
...
1. Встановлення LibreOffice - **`winget install LibreOffice.LibreOffice`**
...
- Створіть папку **Scripts** на диску С.
- Розмістіть проект за шляхом **C:\Scripts**.
- Створіть віртуальне середовище в проекті. Для цього відкрийте проект в ранніше встановленому Visual Studio Code та виконайте цю команду в терміналі - **`python -m venv .venv`**
Після чого в корні проекту має з'явитись папка **.venv**.
...
**Наступні змінні заповніть данними своєї компанії**.<br>
...
Потірбно створити 32 (можна і більше) значний токен (це значення використовується в [Power Automate](https://make.powerautomate.com/), [налаштування Power Automate](#налаштування-power-automate) буде нижче).<br>

Сгенерувати можна за допомогою Python скрипта нижче
```python
import secrets
...
Потрібно зареєструвати додаток в [Entra](https://entra.microsoft.com/) -> App registrations. Сгенерувати ключ в розділі Certificates & secrets (ключ генерується на визначений термін, після закінчення терміну діїї ключа його потрібно перегенерувати і додати нове значення в файл **".env"** в змінну `CLIENT_SECRET=` , інакше додаток працювати небуде) Certificates & secrets (client secret) **обов'язкового як тільки сгенеруєте ключ збережіть значення Value (Entra покаже його тільки 1 раз)**. Та надати права, для цього потрібно перейти в розділ API permissions натиснути Add a permission -> Microsoft Graph обрати потрібний тип і дати наступні права:
...
Щоб налаштувати автоматичний запуск скрипта потрібно зайти в планувальник задач. Натискаємо **Win + R** вводимо **taskschd.msc**. У відкрившомуся вікні натискаємо **Create Task...**<br>
...
- Settings
    - Program/script: Вказуємо повний шляд до файлу <b>run_automationAD.bat</b>
...
Налаштовуємо як на скріншотах
![](Images\1.jpg)


Далі йдемо в вкладку **Triggers**, там натискаємо **New** і налаштовуємо як на скріншоті
![](Images\2.jpg)


Далі йдемо в вкладку **Action** там натискаємо **New** і налаштовуємо як на скріншоті


У полі **Program/script:** треба вказати шлях до файлу **python.exe** він знаходиться в **automatizationAD\.venv\Scripts\python.exe**.
У полі **Add arguments (optional):** вкажіть шлях до файлу вашого основного скрипту, **main.py** шлях має бути в кавичках.
У полі **Start in (optional):** вкажіть шлях до папки вашого проєкту БЕЗ лапок (навіть якщо в шляху є пробіли).
![](Images\3.jpg)
...
В пjлі Ruleset name вводимо назву правила. В Discription напишіть що робить (можна залишити пустим), наприклад Захист від DDoS та перебору для автоматизації АД.
...
В розділі When incoming requests match… потібно заповнити поля так 

| Field    | Operator | Value |
| -------- |:-------- |:----- |
| URI Path | equals   | впишіть адрессу, наприклад "/webhook power-automate" |
And
...
Зходимо в [Microsoft Forms](https://forms.cloud.microsoft/) і створюємо нову форму, (потім будемо [обробляти](#налаштування-power-automate) їх в Power Automate).</br>
...
- В дії "Отримати відомості про форму" вставте тойже id
- В дії "HTTP"
    ...
    Нриклад поля ``` "front_or_back_office":"', trim(outputs('Отримати_відомості_про_відповідь')?['body/r4...']), ```
